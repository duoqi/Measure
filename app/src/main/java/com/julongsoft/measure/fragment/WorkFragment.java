...
public class WorkFragment extends BaseFragment implements PopupWindowWork.OnClickItemListener, View.OnClickListener {
    ...
    private WorkDataForTypeAdapter workDataForTypeAdapter;

    private int page = 1;
    private boolean isNextPageData = true;

    private int type = 0; //  1. 变更申请    3.变更令   2.单价申报


    private List<PeriodListData.ListBean> periodListDatas = new ArrayList<>();
    private List<PeriodDataForTypeBean.ListBean> PeriodDataForTypeDatas = new ArrayList<>();
        ...
        lv_data_list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if(type == 0){
                    ...
                    PeriodDataForTypeBean.ListBean period = PeriodDataForTypeDatas.get(position);
                    Intent intent = new Intent(mActivity, WaitAuditActivity.class);
                    intent.putExtra("periodId", period.getId());
                    intent.putExtra("type", type);
                    intent.putExtra("PeriodDataForTypeDatas", PeriodDataForTypeDatas.get(position));
                    Print.e("periodId", period.getId());
                    startActivity(intent);
                }
                ...
                switch (scrollState) {
                    case AbsListView.OnScrollListener.SCROLL_STATE_IDLE:// 停止的状态
                        ...
                        if (type == 0) {
                            ...
                            if (null != PeriodDataForTypeDatas) {
                                if (isNextPageData) {
                                    getDataFromServerForType(++page, segmentId, num, sectionState, type);
                                } else {
                                    tvFooter.setText("没有更多数据");
                                }
    ...
    public void getDataFromServerForType(final int page, String segmentId, String num, String state, int type) {


        HttpManager.getInstance().getHttpService().getListDataForType(user.getToken(), type, page, segmentId, num, state).enqueue(new HttpResultCallback<BaseResultData<PeriodDataForTypeBean>>() {
            @Override
            public void onSuc(Response<BaseResultData<PeriodDataForTypeBean>> response) {
                ...
                if (page == 1) {
                    PeriodDataForTypeDatas = response.body().getContent().getList();
                    workDataForTypeAdapter = new WorkDataForTypeAdapter(PeriodDataForTypeDatas);

                    Print.e(TAG + "_____________________", periodListDatas.size());
                    lv_data_list.setAdapter(workDataForTypeAdapter);
                } else {
                    PeriodDataForTypeDatas.addAll(response.body().getContent().getList());
                    workDataForTypeAdapter.notifyDataSetChanged();
                }
            }
    ...
    @Override
    public void onClickBianGengLing() {
        if (ivRight.getVisibility() == View.VISIBLE) {
            ivRight.setVisibility(View.GONE);
        }
        tv_name.setText("变更令");
    ...
    @Override
    public void onClickDanJiaShenBao() {
        if (ivRight.getVisibility() == View.VISIBLE) {
            ivRight.setVisibility(View.GONE);
        }
        ...
        btn_repirt.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(type == 0){
                    ...
                    getDataFromServerForType(1, null, null, null, type);
                }
                popupWindow.dismiss();
            }
        });
        ...
        btn_confirm.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(type == 0){
                    ...
                    getDataFromServerForType(1, segmentId, num, sectionState, type);
                }
//                getDataFromServer(1, segmentId, num, sectionState);
...
//                }
    ...
    private class WorkDataForTypeAdapter extends BaseAdapter {
        private List<PeriodDataForTypeBean.ListBean> periodListDatas;

        public WorkDataForTypeAdapter(List<PeriodDataForTypeBean.ListBean> _periodListDatas) {
            periodListDatas = _periodListDatas;
        }
        ...
        @Override
        public View getView(int position, View convertView, ViewGroup parent) {
            ViewHolderForData holder;
            if (null == convertView) {
                convertView = View.inflate(mActivity, R.layout.item_work_data, null);
                holder = new ViewHolderForData();
                holder.rlNumber = (RelativeLayout) convertView.findViewById(R.id.rl_number);
                holder.tv_title = (TextView) convertView.findViewById(tv_title);
                holder.tv_state = (TextView) convertView.findViewById(R.id.tv_state);
                holder.tv_time = (TextView) convertView.findViewById(R.id.tv_time);
                ...
                holder = (ViewHolderForData) convertView.getTag();
            }
            holder.tv_number.setText(position + 1 + "");
            ...
            if(type == 1){
                ...
                if(null != periodListDatas.get(position).getCode()){
                    holder.tv_title.setText("变更申请编号："+ periodListDatas.get(position).getCode());
                }

                if(null != periodListDatas.get(position).getStateStr()){
                    holder.tv_state.setText(periodListDatas.get(position).getStateStr()+"");
                }

                if(null != periodListDatas.get(position).getState()){
                    if (periodListDatas.get(position).getState().equals("0")) {
                        holder.tv_state.setTextColor(getResources().getColor(R.color.workState));
                        holder.rlNumber.setBackground(getResources().getDrawable(R.drawable.shape_work_number_two));
                    } else if (periodListDatas.get(position).getState().equals("1")) {
                        holder.tv_state.setTextColor(getResources().getColor(R.color.colorTitle));
                        holder.rlNumber.setBackground(getResources().getDrawable(R.drawable.shape_work_number_three));
                    } else if (periodListDatas.get(position).getState().equals("2")) {
                        holder.tv_state.setTextColor(getResources().getColor(R.color.workStateRight));
                        holder.rlNumber.setBackground(getResources().getDrawable(R.drawable.shape_work_number_four));
                    } else if (periodListDatas.get(position).getState().equals("-1")) {
                        holder.tv_state.setTextColor(getResources().getColor(R.color.workState));
            ...
            if(type == 2){ //单价申报
                ...
                if(null != periodListDatas.get(position).getCode()){
                    holder.tv_title.setText("单价申报编号："+ periodListDatas.get(position).getCode());
                }
            ...
            if(type == 3){ //变更令
                ...
                if(null != periodListDatas.get(position).getCode()){
                    holder.tv_title.setText("变更令号："+ periodListDatas.get(position).getCode());
                }
